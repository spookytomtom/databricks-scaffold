...
- **`core.py` — `VolumeSpiller`**: Central class managing the Parquet spill buffer.
  ...
  - `drop_on_error` constructor flag (default `False`): when `True`, installs an IPython custom exception handler and `atexit` backstop that call `teardown()` automatically on any uncaught notebook error or at process exit. Drops the volume in **both** dev and prod modes (overrides the default dev-preserve behavior). Eliminates the need for `try/finally` boilerplate in notebooks. Under Databricks Connect as a plain script (no IPython kernel), only the `atexit` backstop fires — use `with`/`try/finally` in that scenario.
